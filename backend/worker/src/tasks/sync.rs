...
pub fn verify_skill(
    ...
    let obj = yaml_value.as_mapping()
        .ok_or_else(|| anyhow::anyhow!("Frontmatter is not a YAML mapping"))?;
    
    let allowed_keys = ["name", "description", "license", "compatibility", "allowed-tools", "metadata"];
    for key in obj.keys() {
    ...
    if !n.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || c == '-') {
        return Err(anyhow::anyhow!("Skill name must only contain lowercase letters, numbers, and hyphens"));
    }
    if n.starts_with('-') || n.ends_with('-') {
        return Err(anyhow::anyhow!("Skill name must not start or end with a hyphen"));
    }
    if n.contains("--") {
        return Err(anyhow::anyhow!("Skill name must not contain consecutive hyphens"));
    }
...
async fn process_repo(
    ...
    let zip_data = match github.download_zipball(&repo.owner, &repo.name).await {
        Ok(data) => data,
        Err(e) => {
            tracing::warn!("Failed to download zip for {}/{}: {}", repo.owner, repo.name, e);
            return Err(e);
        }
    };
    ...
    if parts.len() < 3 {
        return blacklist_repo(db, repo, "Invalid SKILL.md format (missing frontmatter separators)").await;
    }
    ...
    let version_str = frontmatter.metadata
        .as_ref()
        .and_then(|m| m.get("version"))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .unwrap_or_else(|| {
             format!("0.0.{}", chrono::Utc::now().timestamp())
        });

    tracing::info!("Updating skill {} to version {}", frontmatter.name, version_str);
...
async fn blacklist_repo(
    ...
    if let Err(e) = blacklist_entry.insert(db).await {
         tracing::warn!("Failed to insert blacklist entry (might already exist): {}", e);
    }

    // Remove from skill_registry
    let res = skill_registry::Entity::delete_by_id(repo.id).exec(db).await?;
    if res.rows_affected > 0 {
...
mod tests {
    ...
    use common::entities::{skill_registry, skills, skill_versions};
    use std::io::Write;
...
# Test Skill Body").unwrap();
    ...
    async fn test_sync_new_skill() -> Result<()> {
        ...
        let db = db.append_exec_results(vec![
                ...
                MockExecResult { last_insert_id: 100, rows_affected: 1 },
            ])
            .into_connection();
        ...
        github.expect_download_zipball()
...
";
    ...
    fn test_package_skill() {
        ...
        archive.by_name("SKILL.md").unwrap().read_to_string(&mut s).unwrap();
        assert_eq!(s, "content");
    }
}
